...
interface HeroVideoContainerProps {
  duration?: number; // milliseconds for crossfade timing
}
...
export default function HeroVideoContainer({
  ...
  return (
    <section className="relative w-full h-[60vh] md:h-[75vh] overflow-hidden">
      ...
      {/* Animated overlay */}
      <AnimatedTextOverlay
        key={currentIndex} // still remounts overlay for animation
        headline={videos[currentIndex].headline}
        subtext={videos[currentIndex].subtext}
        ctaText={videos[currentIndex].ctaText}
        onCtaClick={() =>
          console.log("CTA clicked:", videos[currentIndex].ctaText)
        }
      />
    </section>
  );
